...
//    int raco_counter = maxEvaluations * 0.5;
    ...
    for(int it = 0; it < maxEvaluations/conf->popSize; it++){
        /// Realiza cross e mut
        Operate();
    ...
    plot_file.open(conf->output + std::to_string(conf->seed) + ".plot", std::ios_base::app);
...
//    for(int i = 0; i < conf->popSize; i++){
    for(int i = 0; i < 1; i++){
        cout << conf->seed << ";" << std::chrono::duration_cast<std::chrono::milliseconds>(end - beg).count() << ";" << pop[i]->fitness << ";" << pop[i]->fitnessTest << ";" << pop[i]->fitnessValidation << ";";
        pop[i]->print();
//        cin.get();
    }
    ...
    hist_file.open(conf->output + std::to_string(conf->seed) + ".hist", std::ios_base::app);
//    std::streambuf *coutbuf = std::cout.rdbuf(); //save old buf
