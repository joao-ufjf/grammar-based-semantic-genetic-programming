...
//    sort(semanticTargetedSubTrees.begin(), semanticTargetedSubTrees.end(), compareSemantic);
    if(semanticTargetedSubTrees.size() > 0){
        ...
        if(semanticTargetedSubTrees.at(rand_int)->semantic_distance >= conf->lower_bound_sensitivity &&
           semanticTargetedSubTrees.at(rand_int)->semantic_distance <= conf->upper_bound_sensitivity){
            m = semanticTargetedSubTrees.at(rand_int);
//            cout << rand_int;
//            cin.get();
        }
